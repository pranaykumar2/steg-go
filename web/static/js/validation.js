...
class FormValidator {
    ...
    initialize() {
        ...
        // Create the result containers if they don't exist
        this.setupResultContainers();

        // Set up event listeners
        this.setupEventListeners();

        // Initial validation of all forms
        this.validateAllForms();

        // Setup direct message event monitoring
    ...
    setupResultContainers() {
        // Create Hide File result container
        this.createHideFileResults();

        // Create Extract result container
        this.createExtractResults();

        // Create Analyze result container
        this.createAnalyzeResults();
    }

    /**
     * Create Hide File results section if it doesn't exist
     */
    createHideFileResults() {
        ...
        if (!resultsSection) {
            ...
            // Find the hide-file-form to append after it
    ...
    /**
     * Create Extract results section if it doesn't exist
     */
    createExtractResults() {
        ...
        if (!resultsSection) {
            ...
            // Find the extract-form to append after it
    ...
    /**
     * Create Analyze results section if it doesn't exist
     */
    createAnalyzeResults() {
        ...
        if (!resultsSection) {
            ...
            // Find the analyze-form to append after it
    ...
    /**
     * Set up direct input monitoring for critical fields
     * This is a backup to the regular event listeners
     */
    setupDirectInputMonitoring() {
        // For the hide-text-message field, set up interval checking
        setInterval(() => {
            ...
            if (messageField) {
                ...
                // If validity changed, update and revalidate
    ...
    setupEventListeners() {
        ...
        document.addEventListener('file:uploaded', (e) => {
            ...
            // Special handling for section "hide" - map to hide-text
        ...
        // Text field validation - add multiple event types
        const messageField = document.getElementById('hide-text-message');
        if (messageField) {
            ...
            // Try setting an initial value to trigger validation
            setTimeout(() => {
        ...
        // Add debugging button to check validation state
        this.addDebugTools();
    ...
    addDebugTools() {
        if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
            document.addEventListener('keydown', e => {
                ...
                // Press Alt+R to show all result sections (for debugging)
                if (e.altKey && e.key === 'r') {
    ...
    /**
     * Highlight forms for debugging
     */
    highlightFormsDebug() {
        // Add temporary highlight to show form validation state
    ...
    handleHideFileSuccess(response) {
        ...
        `;
                ...
                // Fallback to basic file details if fileDetails isn't available
    ...
    handleExtractSuccess(response) {
        ...
        // Get specific result containers
        ...
        if (!data.isFile && data.message) {
            ...
            // Show text results, hide file results
            ...
            // Show file results, hide text results
        ...
        `;
                ...
                if (image) {
                    ...
                    img.onload = () => {
                        ...
                        const charCapacity = Math.floor(byteCapacity * 0.8); // Rough estimate for characters
                        const wordCapacity = Math.floor(charCapacity / 6); // Average word length
    ...
    getFile(section, type = 'image') {
        // Handle special mapping for 'hide' section
        const mappedSection = section === 'hide' ? 'hide-text' : section;

        // Use fileHandler to get the file
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  `;
    ...
    // Emergency fix - force validation to run after a short delay for any pre-filled fields
    setTimeout(() => {
        // Check message field
        const messageField = document.getElementById('hide-text-message');
